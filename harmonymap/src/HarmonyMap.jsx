...
const CT={major:{iv:[0,4,7],q:'major'},minor:{iv:[0,3,7],q:'minor'},dim:{iv:[0,3,6],q:'diminished'},aug:{iv:[0,4,8],q:'augmented'},dom7:{iv:[0,4,7,10],q:'dominant'},maj7:{iv:[0,4,7,11],q:'major'},min7:{iv:[0,3,7,10],q:'minor'},'m7b5':{iv:[0,3,6,10],q:'diminished'},sus2:{iv:[0,2,7],q:'suspended'},sus4:{iv:[0,5,7],q:'suspended'},power:{iv:[0,7],q:'power'}};
...
function pc(sym){if(!sym)return{r:'C',t:'major'};const m=sym.match(/^([A-G][#b]?)(m7b5|m7|maj7|sus2|sus4|°|m|7|5)?$/);if(!m)return{r:'C',t:'major'};const M={'':'major',m:'minor','°':'dim','7':'dom7',maj7:'maj7',m7:'min7',m7b5:'m7b5',sus2:'sus2',sus4:'sus4','5':'power'};return{r:m[1],t:M[m[2]||'']||'major'};}
function cc(s){const q=CT[pc(s).t]?.q;return{major:'#FF6B9D',minor:'#4FD1C7',diminished:'#C77DFF',dominant:'#FF9500',suspended:'#5EEAD4',power:'#94A3B8'}[q]||'#fff';}
...
const MINOR_COF=['A minor','E minor','B minor','F# minor','C# minor','D minor','G minor','C minor','F minor','Bb minor','Eb minor','Ab minor'].filter(k=>KEYS[k]);

function chordRN(k,ch){if(!k?.ch)return'';const rn=['I','ii','iii','IV','V','vi','vii°'],rm=['i','ii°','III','iv','v','VI','VII'];const pos=k.ch.indexOf(ch);return pos===-1?'':(k.m==='minor'?rm[pos]:rn[pos]);}
function gcon(ch,mode='major'){if(!ch||ch.length<7)return[];const p=mode==='minor'?[[0,3],[0,4],[0,5],[1,4],[3,0],[3,4],[4,0],[4,5],[5,3],[5,2]]:[[0,3],[0,4],[0,5],[1,4],[3,0],[3,4],[4,0],[4,5],[5,3]];return p.map(([a,b])=>({f:ch[a],t:ch[b],st:((a===4&&b===0)||(a===3&&b===0))?'strong':'normal'}));}
